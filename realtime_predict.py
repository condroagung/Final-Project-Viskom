...
def show_results(img, results, img_width, img_height, model_age, model_gender, model_emotion):
    ...
    for i in range(len(results)):
        ...
        cv2.rectangle(img_cp, (xmin, ymin-20),
                      (xmax, ymin), (125, 125, 125), -1)
        cv2.putText(img_cp, results[i][0] + ' : %.2f' % results[i][5],
                    (xmin+5, ymin-7), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)

        target_image = img_cp

        # analyze detected face
        xmin2, xmax2, ymin2, ymax2 = crop(
            x, y, w, h, 1.1, img_width, img_height)
        ...
        cv2.rectangle(target_image, (xmin2, ymin2),
                      (xmax2, ymax2), color=(0, 0, 255), thickness=3)
        ...
        if(model_age != None):
            ...
            cv2.putText(target_image, "Age : "+label, (xmin2, ymax2+offset),
                        cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (0, 0, 250))
            offset = offset+16

        if(model_gender != None):
            shape = model_gender.layers[0].get_output_at(
                0).get_shape().as_list()
            ...
            cv2.putText(target_image, "Gender : %.2f" % prob_gender_keras + " " +
                        lines_gender[cls_gender_keras], (xmin2, ymax2+offset), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (0, 0, 250))
            offset = offset+16

        if(model_emotion != None):
            shape = model_emotion.layers[0].get_output_at(
                0).get_shape().as_list()
            ...
            cv2.putText(target_image, "Emotion : %.2f" % prob_emotion_keras + " " +
                        lines_fer2013[cls_emotion_keras], (xmin2, ymax2+offset), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (0, 0, 250))
            offset = offset+16
...
def main(argv):
    ...
    model_gender = load_model(
        MODEL_ROOT_PATH+'agegender_gender_squeezenet.hdf5')
    if(os.path.exists(MODEL_ROOT_PATH+'fer2013_mini_XCEPTION.102-0.66.hdf5')):
        model_emotion = load_model(
            MODEL_ROOT_PATH+'fer2013_mini_XCEPTION.102-0.66.hdf5')
    else:
        model_emotion = None
    ...
    while True:
        ...
        show_results(
            img_cv, results, img.shape[1], img.shape[0], model_age, model_gender, model_emotion)
